...
DMAcontext* DMA_create(ibv_device* ib_dev, int thread_id, bool isPS) {
  ...
  struct ibv_cq* rec_cq =
      ibv_create_cq(context, my_recv_queue_length + 1, NULL, NULL, 0);
  if (!rec_cq) {
  ...
  struct ibv_cq* snd_cq =
      ibv_create_cq(context, my_send_queue_length + 1, NULL, NULL, 0);
  if (!snd_cq) {
    fprintf(stderr, "Couldn't create CQ %d\n", errno);
    exit(1);
  }
  ...
  struct ibv_exp_qp_init_attr* qp_init_attr =
      (struct ibv_exp_qp_init_attr*)malloc(sizeof(struct ibv_exp_qp_init_attr));
  ...
  // send_buf = alloc_raw_pages(send_buf_size / EACH_HUGEPAGE_SIZE + 1,
  // EACH_HUGEPAGE_SIZE);
  ...
  struct ibv_cq* mp_recv_cq = ibv_exp_create_cq(
      context, kAppRecvCQDepth / 2, nullptr, nullptr, 0, &cq_init_attr);
  ...
  struct ibv_exp_wq_family* mp_wq_family =
      reinterpret_cast<struct ibv_exp_wq_family*>(
  ...
  struct ibv_exp_rwq_ind_table* mp_ind_tbl =
      ibv_exp_create_rwq_ind_table(context, &rwq_ind_table_init_attr);
  ...
  mp_qp_init_attr.comp_mask = IBV_EXP_QP_INIT_ATTR_CREATE_FLAGS |
  ...
  struct ibv_mr* mp_send_mr =
      ibv_reg_mr(pd, mp_send_ring, tx_ring_size, IBV_ACCESS_LOCAL_WRITE);
  ...
  struct ibv_mr* mp_mr =
      ibv_reg_mr(pd, mp_recv_ring, kAppRingSize, IBV_ACCESS_LOCAL_WRITE);
  ...
  for (size_t i = 0; i < kAppRecvCQDepth; i++) {
    ...
    mp_cqe_arr[i].wqe_counter =
        htons(kAppStridesPerWQE - (kAppRecvCQDepth - i));
  ...
  struct ibv_sge* mp_sge = reinterpret_cast<struct ibv_sge*>(
      malloc(sizeof(struct ibv_sge) * kAppRQDepth));
  ...
  printf(
      "kAppRingMbufSize=%lu, kAppStridesPerWQE=%lu, kAppRingSize=%lu, "
      "kAppRQDepth=%lu\n",
...
void send_packet(DMAcontext* dma_context, int chunk_size, uint64_t offset) {
  ...
  sg.addr =
      (uintptr_t)((char*)dma_context->send_region + offset * P4ML_LAYER_SIZE);
  // printf("%d\n", sg.addr);
  ...
  if (dma_context->isPS)
    memcpy(hdr, PS_IP_ETH_UDP_HEADER,
           IP_ETH_UDP_HEADER_SIZE);  // Assuming that the header buffer was
                                     // define before.
  else
    memcpy(hdr, WORKER_IP_ETH_UDP_HEADER,
           IP_ETH_UDP_HEADER_SIZE);  // Assuming that the header buffer was
                                     // define before.

  hdr[5] = dma_context->id;
  // hdr[37] = dma_context->id;
  wr.tso.hdr = hdr;  // There is no need to use malloc operation in this case,
                     // local definition of hdr is ok.
  ...
  if (DEBUG_PRINT_ALL_SENDING_PACKET)
    for (int i = 0; i < chunk_size / P4ML_LAYER_SIZE; i++)
      p4ml_header_print_h((agghdr*)((char*)sg.addr + i * P4ML_LAYER_SIZE),
                          "SEND");
  ...
  if (dma_context->isMarkTimeStamp) {
    ...
    for (int i = 0; i < chunk_size / P4ML_LAYER_SIZE; i++) {
      ...
      if (!dma_context->isSent[ntohs(p4ml_header->seq_num)]) {
        ...
        dma_context->first_send_time[ntohs(p4ml_header->seq_num)] =
            current_time;
      } else {
        /* Resend may trigger */
      }
...
size_t receive_packet(DMAcontext* dma_context, cqe_snapshot_t* new_snapshot) {
  ...
  const size_t delta =
      get_cycle_delta(dma_context->prev_snapshot, *new_snapshot);

  if (!(delta == 0 || delta >= kAppNumRingEntries)) {
    if (DEBUG_CHECK_SEND_RECEIVE_TOTAL) dma_context->total_received += delta;
    return delta;
  } else
    return 0;
  // return delta;
...
void dma_postback(DMAcontext* dma_context) {
  ...
  if (dma_context->nb_rx_rolling == kAppStridesPerWQE) {
    ...
    int ret = dma_context->mp_wq_family->recv_burst(
        dma_context->mp_wq, &dma_context->mp_sge[dma_context->sge_idx], 1);
    rt_assert(ret == 0);
...
void install_flow_rule(struct ibv_qp* qp, uint16_t thread_id, bool isPS) {
  static constexpr size_t rule_sz = sizeof(ibv_exp_flow_attr) +
                                    sizeof(ibv_exp_flow_spec_eth) +
