...
inline char* alloc_raw_pages(int cnt, int size) {
  ...
  char* ptr = (char*)mmap(NULL, (int64_t)cnt * size, PROT_READ | PROT_WRITE,
                          flag, -1, 0);
  if (ptr == (char*)-1) {
...
// /* Returns the MAC Address Params: int iNetType - 0: ethernet, 1: Wifi char
// chMAC[6] - MAC Address in binary format Returns: 0: success -1: Failure */
...
// /* Returns the interface IP Address Params: int iNetType - 0: ethernet, 1:
// Wifi char *chIP - IP Address string Return: 0: success / -1: Failure */ int
// getIpAddress(char chIP[16])
// {
//     struct ifreq ifr;
//     int sock = 0;
//     sock = socket(AF_INET, SOCK_DGRAM, 0);
...
//         return -1;
//     }
//     sprintf(chIP, "%s", inet_ntoa(((struct
//     sockaddr_in*)&(ifr.ifr_addr))->sin_addr)); close(sock); return 0;
// }
