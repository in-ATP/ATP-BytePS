...
struct PushPullParam {
  ...
  int priority;
  uint64_t p4ml_key;
  uint64_t p4ml_enqueue_num;

  PushPullParam(BPSContext* context, std::shared_ptr<NDArray> input,
                int version, int priority, uint64_t p4ml_key,
                uint64_t p4ml_enqueue_num)
      : context(context),
        input(input),
        version(version),
        priority(priority),
        p4ml_key(p4ml_key),
        p4ml_enqueue_num(p4ml_enqueue_num) {}
};
...
void DoPushPull(void*, void* on_complete_ptr, void* param) {
  ...
  auto enqueue_result = common::EnqueueTensor(
      ...
      queue_list, push_pull_param->p4ml_key, push_pull_param->p4ml_enqueue_num);
...
extern "C" int byteps_mxnet_push_pull_async(NDArray* tensor, char* name,
  ...
  uint64_t p4ml_key;
  uint64_t p4ml_enqueue_num;
  int64_t accumulated = size;
  while (accumulated > 0) {
    p4ml_key = BytePSGlobal::GetP4MLkey();
    p4ml_enqueue_num = BytePSGlobal::GetEnqNumber();
    accumulated -= BytePSGlobal::GetPartitionBound();
  }

  auto push_pull_param = new PushPullParam(
      &context, tensor_copy, version, priority, p4ml_key, p4ml_enqueue_num);
  auto var = tensor->var();
